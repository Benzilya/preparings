  ...
  return (
    <div className="progressPage">
      ...
      <section className="metrics" aria-label={copy.summary}>
        <article className="metric"><strong>{completion}%</strong><span>{copy.completed}</span></article>
        <article className="metric"><strong>{summary.learning}</strong><span>{copy.learning}</span></article>
        <article className="metric"><strong>{summary.favorites}</strong><span>{copy.favorites}</span></article>
      </section>

      <section className="progressPanel">
        <div className="progressPanelHeader">
          <div><p className="cardLabel">{copy.backup}</p><h2>{copy.importExport}</h2></div>
          <div className="progressActions">
            <button onClick={downloadProgress} type="button">{copy.exportJson}</button>
            <label className="progressImport">{copy.importJson}<input accept="application/json,.json" onChange={handleImport} type="file" /></label>
            {!resetPending ? <button onClick={() => setResetPending(true)} type="button">{copy.reset}</button> : null}
          </div>
        </div>
        {resetPending ? (
          <div className="resetConfirmation" role="alert">
            <div>
              <strong>{copy.deleteTitle}</strong><p>{copy.deleteHint}</p>
              <label className="resetConfirmationField"><span>{copy.confirmation}</span><input autoComplete="off" onChange={(event) => setResetConfirmation(event.target.value)} placeholder={resetConfirmationText} value={resetConfirmation} /></label>
            </div>
            <div className="progressActions">
              <button onClick={cancelReset} type="button">{copy.cancel}</button>
              <button className="dangerAction" disabled={resetConfirmation !== resetConfirmationText} onClick={confirmReset} type="button">{copy.deleteProgress}</button>
            </div>
          </div>
        ) : null}
      ...
      <section className="progressPanel">
        <div className="progressPanelHeader"><div><p className="cardLabel">{copy.categories}</p><h2>{copy.coverage}</h2></div></div>
          ...
          {categoryStats.map((item) => {
            ...
            return (
              <article className="categoryProgressItem" key={item.category}>
                <div><strong>{item.category}</strong><span>{item.completed} {copy.completedShort} · {item.learning} {copy.learningShort} · {item.total} {copy.totalShort}</span></div>
      ...
      <section className="progressPanel">
        <div className="progressPanelHeader"><div><p className="cardLabel">{copy.activity}</p><h2>{copy.history}</h2></div></div>
                ...
                <div className="recentProgressList">
                  {items.map(({ record, question }) => (
                    <Link href={`/questions/${question.slug}`} key={record.questionId}>
                      ...
                      <span>{statusLabel(record.status)}{record.favorite ? ` · ${copy.favorite}` : ""} · {new Date(record.updatedAt).toLocaleTimeString(copy.locale, { hour: "2-digit", minute: "2-digit" })}</span>
                    </Link>
                  ))}
                </div>
              </section>
