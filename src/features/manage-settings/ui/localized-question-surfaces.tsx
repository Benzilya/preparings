...
export function LocalizedCategoriesPage({
  categories,
}: {
  categories: readonly QuestionCategorySummary[];
}) {
      ...
      <div className="questionGrid">
        {localizedCategories.map((category) => (
          <Card key={category.slug}>
            ...
            <CardContent>
              <p>
                {category.count} {category.count === 1 ? copy.question : copy.questions}
              </p>
  ...
  return (
    <article className="questionDetails">
      ...
      <section className="detailSection">
        <h2>{copy.interviewerGoal}</h2>
        <p>{localized.interviewerGoal}</p>
      </section>
      <section className="detailSection">
        <h2>{copy.expectedAnswer}</h2>
        <p>{localized.expectedAnswer}</p>
      </section>
      <section className="detailSection">
        <h2>{contentCopy.details.alternativeAnswers}</h2>
        <ul>
          {localized.alternativeAnswers.map((answer) => (
        ...
        <div className="answerExampleGrid">
          {localized.answerExamples.map((example) => (
            <div className="answerExample" key={example.level}>
              <strong>{contentCopy.difficulty[example.level]}</strong>
              <p>{example.answer}</p>
            </div>
          ))}
        </div>
      ...
      <div className="detailColumns">
        ...
        <section className="detailSection">
          ...
          <ul>
            {localized.followUpQuestions.map((followUp) => (
      ...
      <section className="detailSection">
        <h2>{contentCopy.details.relatedTopics}</h2>
        <ul className="tagList">
          {localized.relatedTopics.map((topic) => (
            <li key={topic}>{topic}</li>
          ))}
        </ul>
      </section>
      {localized.practicalExample ? (
        <section className="detailSection detailHighlight">
          <h2>{copy.practicalExample}</h2>
          <p>{localized.practicalExample}</p>
        </section>
      ) : null}
      {localized.experienceExample ? (
        <section className="detailSection detailHighlight">
          <h2>{contentCopy.details.experienceExample}</h2>
          <p>{localized.experienceExample}</p>
        </section>
      ) : null}
      <section className="detailSection">
        <h2>{copy.sources}</h2>
        <ul className="sourceList">
          {localized.sources.map((source) => (
            <li key={source.url}>
