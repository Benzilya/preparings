...
export function generateStaticParams() {
  return [...new Set(top100Questions.map((question) => question.categorySlug))].map((categorySlug) => ({ categorySlug }));
}

export default async function QuestionCategoryPage({ params }: { params: Promise<{ categorySlug: string }> }) {
  const { categorySlug } = await params;
  const questions = top100Questions
    .filter((question) => question.categorySlug === categorySlug)
    .toSorted((left, right) => left.popularityRank - right.popularityRank || left.slug.localeCompare(right.slug, "en"));
